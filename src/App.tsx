...
const skillColor = (cat: string) =>
  `var(--skill-palette-${categoryOrder.indexOf(cat) % SKILL_PALETTE_SIZE})`;

const clamp = (value: number, min: number, max: number) =>
  Math.min(max, Math.max(min, value));

const getPageGutter = (width: number, height: number) =>
  clamp(
    Math.min(width, height) * PAGE_GUTTER_RATIO,
    MIN_PAGE_GUTTER,
    MAX_PAGE_GUTTER,
  );
...
const getCardScale = (width: number, height: number) => {
  ...
  return Math.min(
    1,
    availableWidth / CARD_BASE_WIDTH,
    availableHeight / CARD_BASE_HEIGHT,
  );
};
...
function App() {
  ...
  const [cardScale, setCardScale] = useState(() =>
    getCardScale(window.innerWidth, window.innerHeight),
  );
  ...
  useEffect(() => {
    ...
    const resizeObserver =
      typeof ResizeObserver !== "undefined" && page
        ? new ResizeObserver(updateCardScale)
        : null;
        ...
        <div className="card-scene">
          <div className="card-scale">
            ...
            >
              {/* ── FRONT ── */}
              <div
                className={`card-face card-front ${flipped ? "is-inactive" : "is-active"}`}
                aria-hidden={flipped}
              >
              ...
              {/* ── BACK ── */}
              <div
                className={`card-face card-back ${flipped ? "is-active" : "is-inactive"}`}
                aria-hidden={!flipped}
              >
                <div className="bg-accent"></div>
                    ...
                    {skills.map((skill, i) => (
                      <span
                        ...
                        style={
                          {
                            "--i": i,
                            "--skill-color": skillColor(skill.category),
                          } as CSSProperties
                        }
                      >
                        {skill.name}
