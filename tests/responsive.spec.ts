...
for (const viewport of viewports) {
  test.describe(`${viewport.name} layout`, () => {
    ...
    test('keeps the card visible without page overflow', async ({ page }) => {
      ...
      expect(hintBox.y + hintBox.height).toBeLessThanOrEqual(viewportSize.height);
      ...
      expect(pageMetrics.documentScrollWidth).toBeLessThanOrEqual(pageMetrics.documentClientWidth + 1);
      expect(pageMetrics.documentScrollHeight).toBeLessThanOrEqual(pageMetrics.documentClientHeight + 1);
    ...
    test('keeps flip interaction working after resize-sensitive layout calculations', async ({ page }) => {
      await page.goto('/');
