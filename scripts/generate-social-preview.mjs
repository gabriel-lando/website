...
const server = createServer(async (request, response) => {
  try {
    ...
    response.end(
      error instanceof Error ? error.message : "Failed to serve build output.",
    );
  }
});
...
const launchBrowser = async () => {
  try {
    ...
    throw new Error(
      `Chromium is required to generate social-preview.png. Run \"npm run playwright:install -- chromium\" first.\n${error instanceof Error ? error.message : String(error)}`,
    );
  }
};
...
try {
  ...
  await page.locator(".front-photo img").evaluate((image) => {
    ...
    return new Promise((resolveImage) => {
      image.addEventListener("load", () => resolveImage(undefined), {
        once: true,
      });
      image.addEventListener("error", () => resolveImage(undefined), {
        once: true,
      });
    });
  });
  ...
  if (!box) {
    throw new Error(
      "Unable to locate the front card for social preview generation.",
    );
  }
